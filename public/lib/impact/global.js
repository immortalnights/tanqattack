(function (window) {
    ...
    window.ig = {
        ...
        merge: function (original, extended) {
            ...
            //~ console.log(org, '+', extended, '=', original);
            return original;
        },
        ksort: function (obj) {
    ...
    window.ig.Class.extend = function (prop) {
        ...
        Class.implements = window.ig.Class.implements;
        Class.inject = inject;
        return Class;
    };
    window.ig.Class.implements = function (prop) {
        ...
        //! validate the derived class and the interface
        //! checks that all Interface functions have been implemented
        //! checks that the Interface has no member variables
        for (var name in parent)
        {
            if (typeof(parent[name]) == "function")
            {
                //~ console.log("interface property '%s' (type %s):", name, typeof(parent[name]));
                //~ console.log("implement property '%s' (type %s)", name, typeof(prop[name]));
                if (typeof(prop[name]) != "function")
                {
                    console.log("interface property '%s' (type %s)", name, typeof(parent[name]), parent[name]);
                    console.log("implement property '%s' (type %s)", name, typeof(prop[name]), prop[name]);
                    throw("Interface function '"+name+"' has not been implemented");
                }
            }
            else
            {
                console.log("interface property '%s' (type %s):", name, typeof(parent[name]), parent[name]);
                throw("Interface should only define functions");
            }
        }
