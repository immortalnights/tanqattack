			...
			else
			{
				if (actor.mobility !== 'static')
				{
					...
					this.actors.forEach((otherActor) => {
						// can't hit self, can't hit owned, can't hit dead
						if (actor !== otherActor && actor.owner !== otherActor.owner && otherActor.life > 0)
						{
							...
							if (actor.polygon && otherActor.polygon)
							{
								...
								// {
								// 	if (SAT.testPolygonCircle(otherActor.polygon, actor.polygon, response))
								...
								// {
								// 	if (SAT.testPolygonCircle(actor.polygon, otherActor.polygon, response))
								...
								if (actor.polygon instanceof SAT.Circle && otherActor.polygon instanceof SAT.Circle)
								{
									result = SAT.testCircleCircle(actor.polygon, otherActor.polygon, response);
								}
								else if (actor.polygon instanceof SAT.Circle)
								{
									result = SAT.testPolygonCircle(otherActor.polygon, actor.polygon, response);
								}
								else if (otherActor.polygon instanceof SAT.Circle)
								{
									result = SAT.testPolygonCircle(actor.polygon, otherActor.polygon, response);
								}
								else if (actor.polygon instanceof SAT.Polygon && otherActor.polygon instanceof SAT.Polygon)
								{
									result = SAT.testPolygonPolygon(actor.polygon, otherActor.polygon, response);
								}

								if (result && response.overlap !== 0)
								{
									actor.hit(otherActor, response);
								}
